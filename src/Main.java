...
public class Main {
    public static void main(String[] args) throws IOException {
              ...
              if (args.length == 0) {
                  ...
                  menu.printColorLine("--> Enter source file name   --> ");
                  String fileName = scanner.nextLine();
                  checker.fileExist(fileName);

                  menu.printColorLine("--> Enter cipher code (integer number) --> ");
                  ...
                  if (!encryptMode && !decryptMode && !bfMode) {
                      menu.printColorLine("--> Unknown mode! Check mode parameter");
                      menu.sayBye();
                      System.exit(0);
                  }

                  if (args.length < 3 && !bfMode) {
                      menu.printColorLine("--> Not enough parameters!");
                      menu.sayBye();
                      System.exit(0);
                  }
                  ...
                  if (encryptMode) {
                      ...
                      menu.printColorLine("--> File was encrypted with key " + code + " Use scroll to see it!\n");
                  } else if (decryptMode) {
                      if (checker.encryptedFileExist(encryptedFileName, 0)) {
                          code = checker.getCode(args[2]);

                          doIt.caseHandle(pathOut, pathDec, code * (-1));
                          menu.printColorLine("--> File was decrypted with key " + code + " Use scroll to see it!\n");
                      }
                  } else {
                      if ( checker.encryptedFileExist(encryptedFileName, 0)) {
                          ...
                          menu.printColorLine("--> File was decrypted by brute force! Scroll up to see it!\n");
                      }
                  }
                menu.sayBye();
          }
